...
const HTMLEditor = () => {
  ...
  const addVisualization = () => {
    axios.post(`${process.env.NEXT_PUBLIC_API_URL}/dom/add`, { title: 'Untitled Diagram' }, {
      headers: {
        'x-auth-token': token
      }
    })
  ...
  const deleteDom = (id, event) => {
    event.stopPropagation();
    axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/dom/delete/` + id)
      .then((result) => {
        console.log(result.data);
        ...
        <div className='my-10 mx-2 border border-gray-800'>
          {
            diagramList.map(diagram => (
              <div onClick={() => { setSelDiagram(diagram); setCode(diagram.code); }} key={diagram._id} className='border-2 flex gap-5 justify-between items-center p-2'>
                ...
                <button onClick={(event) => deleteDom(diagram._id, event)} className='flex gap-2 justify-between p-2 bg-red-500 py-1 px-3 border border-gray-700 text-white rounded-full'>
                  <IconTrash />
                </button>
              </div>
            ))
